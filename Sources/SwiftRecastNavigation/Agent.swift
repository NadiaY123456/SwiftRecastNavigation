...
public class CrowdAgent: CustomDebugStringConvertible {
    ...
    public var obstacleAvoidanceType: Int {
        ...
        set {
            guard newValue >= 0 && newValue < DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS else { return }
            update { p in p.obstacleAvoidanceType = UInt8(newValue) }
        }
    }
    ...
    public struct Params {
        ...
        ///   - obstacleAvoidanceType: The index of the avoidance configuration to use for the agent (set with ``setObstacleAvoidance(idx:config:)``
