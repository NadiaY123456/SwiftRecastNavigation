...
///
/// Various routines in SwiftRecastNavigation are optimized to use both a polygon reference
...
public class NavMeshQuery {
    ...
    public var query: dtNavMeshQuery
    public var filter: NavQueryFilter

    /// - Parameters:
    ///   - nav: the navigation mesh this will operate on
    ///   - maxNodes: Maximum number of search nodes. [Limits: 0 < value <= 65535]
    public init (nav: NavMesh, maxNodes: Int32 = 2048) throws {
...
public extension NavMeshQuery {
    /// Convenience method: find a straight path from startPos to endPos using an optional filter.
    /// - Throws: NavMesh.NavMeshError on any failure (alloc, init, nearest-poly, corridor, or pull).
    /// - Returns: Array of SIMD3 waypoints.
    func findPath(
        from startPos: SIMD3<Float>,
        to endPos: SIMD3<Float>,
        filter customFilter: NavQueryFilter? = nil
    ) throws -> [SIMD3<Float>] {
        // 1. Snap start/end into the mesh
        let startResult = findNearestPoint(point: startPos, filter: customFilter)
        let (startPoly, _) = try startResult.get()
        let endResult = findNearestPoint(point: endPos, filter: customFilter)
        let (endPoly, _) = try endResult.get()

        // 2. Build the corridor of polygon refs
        let corridorResult = findPathCorridor(filter: customFilter, start: startPoly, end: endPoly)
        let corridor = try corridorResult.get()

        // 3. String‐pull to get a straight path
        let straightResult = findStraightPath(
            filter: customFilter,
            startPos: startPos,
            endPos: endPos,
            pathCorridor: corridor
        )
        let foundPath = try straightResult.get()

        // 4. Convert to [SIMD3<Float>]
        return (0..<foundPath.count).map { foundPath[$0] }
    }
}
